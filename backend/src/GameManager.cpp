...
void GameManager::ClearScreen() {
    // Safe clear screen using newlines to prevent VS Code terminal corruption
    std::cout << "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n"; 
}
...
void GameManager::DrawCharacterSelect() {
    ...
    party.clear(); // Ensure party is empty before drafting
    ...
    for (int i = 0; i < partySize; i++) {
        ...
        // Push the character into the party vector
        party.push_back(std::make_shared<Character>(finalName, std::move(job), 120, 14, 8, 16, 6, 8, 8));
    }

    // Set the designated player pointer for events (defaults to leader)
    player = party[0]; 
...
void GameManager::DrawMerchantUI() {
    ...
    else if (choice == 2 && partyScrap >= 75) { partyScrap -= 75; /* Add energyTonic var to header */ }
...
void GameManager::DrawCampfireUI() {
    ...
    if (choice == 1) {
        ...
        // Attempt to cast the generic Entity* to a Character*
...
void GameManager::ResolveNode() {
    ...
    if (currentType == COMBAT) {
        ...
        else enemies = EnemyFactory::SpawnEnemies(currentTier + 1); // Elites count as +1 tier
        turnCounter = 1;
        SetState(STATE_COMBAT);
    }
        ...
        SetState(STATE_MERCHANT); // Route to the merchant state
    }
    else if (currentType == CAMP) {
        // You can add STATE_CAMP here similarly
        SetState(STATE_MAP); 
    }
    
    // Note: For brevity in this response, Merchant/Camp redirect directly to STATE_MAP. 
    // You can paste the Merchant/Camp functions we built earlier here!
}
...
void GameManager::HandleCombatTurn() {
    ...
    for (auto& combatant : turnOrder) {
        ...
        if (dynamic_cast<Character*>(combatant.get())) {
            // Player Turn Setup (Handled polymorphically inside JobClass)
            std::vector<std::shared_ptr<Entity>> partyRef; 
            partyRef.push_back(player); // Dummy array since it's solo
        ...
        CheckCombatEnd();
        if (currentState != STATE_COMBAT) return; // Exit loop if combat ended
    }
    turnCounter++;
}
...
void GameManager::CheckCombatEnd() {
    ...
    if (allEnemiesDead) {
        ...
        // Grant XP (100 XP per fight ensures fast leveling)
        player->GainXP(100); 
...
// =========================================================
// UI: LEVEL UP (From Progression Mechanics)
// =========================================================
