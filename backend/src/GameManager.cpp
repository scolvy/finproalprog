...
int GetRandomTarget(int numTargets) {
    ...
    std::uniform_int_distribution<int> dist(0, numTargets - 1);
    return dist(rng);
}
...
ClearScreen();
    ...
    LogEvent("SYSTEM START: User '" + localUsername + "' initialized the terminal.");
...
void GameManager::DrawCharacterSelect() {
    ...
    auto DrawClassMenu = []() {
        ...
        std::cout << "================================================================================\n";
        std::cout << "  [1] SCRAPPER   - Frontline Brawler [STR] (High HP, Melee focus)\n";
        std::cout << "  [2] DRIFTER    - Agile Survivor    [DEX] (Balanced, Evasion)\n";
        std::cout << "  [3] SPARKMAGE  - Tech Caster       [INT] (High Damage, Fragile)\n";
        std::cout << "  [4] APOTHECARY - Combat Medic      [WIS] (Healing, Party Buffs)\n";
        std::cout << "  [5] ENVOY      - Tactical Leader   [CHA] (Debuffs, Support)\n";
        std::cout << "--------------------------------------------------------------------------------\n";
...
void GameManager::ResolveNode() {
    ...
    if (currentType == COMBAT) {
        LogEvent("ENCOUNTER: Engaged standard enemies in " + currentBiome + " (Tier " + std::to_string(currentTier) + ").");
...
void GameManager::DrawCampfireUI() {
    ...
    if (choice == 1) {
        ...
        LogEvent("CAMPFIRE: Party rested and restored 30% HP.");
    } 
    else if (choice == 2) {
...
void GameManager::CheckCombatEnd() {
    ...
    if (allEnemiesDead) {
        ...
        partyScrap += scrapWon;
        LogEvent("VICTORY: Defeated enemies. Looted " + std::to_string(scrapWon) + " scrap. Total Scrap: " + std::to_string(partyScrap));
        ...
        if (pChar) {
            if (!network.IsConnected()) {
                ...
                try {
                    ...
                    std::cout << "[!] ERROR: Invalid turn data: " << e.what() << "\n";
                }
            } 
            else {
                ...
                if (network.IsHost()) {
                    // Safely access the clientSockets map
                    auto socketIt = clientSockets.find(pChar->GetName());
                    if (socketIt != clientSockets.end()) {
                        turnMsg = network.ReceiveMessage(socketIt->second);
                        network.BroadcastMessage(turnMsg);
                    } else {
                        std::cout << "[!] ERROR: Client socket not found for " << pChar->GetName() << "\n";
                    }
                } else {
                ...
                try {
                    ...
                    std::cout << "[!] ERROR: Invalid network turn data: " << e.what() << "\n";
                }
            }
        } 
...
void GameManager::ProcessTurnPacket(const std::string& data) {
    std::vector<std::string> tokens = NetworkUtils::Split(data, '|');
    if (tokens.size() < 3) return;

    int choice = std::stoi(tokens[1]);
    int targetIdx = std::stoi(tokens[2]);

    auto actingEntity = std::dynamic_pointer_cast<Character>(party[activeTurnIndex]);

 if (actingEntity) {
        std::vector<std::shared_ptr<Entity>> entityParty(party.begin(), party.end());
        actingEntity->ExecuteNetworkTurn(entityParty, enemies, choice, targetIdx);
    }
    activeTurnIndex = (activeTurnIndex + 1) % party.size();
}
...
void GameManager::DrawMerchantUI() {
    ...
    if (choice == 1 && partyScrap >= 50) { partyScrap -= 50; healthSalves++; 
        LogEvent("MERCHANT: Bought Health Salve. Scrap remaining: " + std::to_string(partyScrap));
    } 
    
    else if (choice == 2 && partyScrap >= 75) { partyScrap -= 75;
        LogEvent("MERCHANT: Bought Energy Tonic. Scrap remaining: " + std::to_string(partyScrap));
    }
...
void GameManager::DrawLevelUpUI() {
    ...
    LogEvent("LEVEL UP: " + player->GetName() + " allocated a stat point (Option " + std::to_string(choice) + ").");
...
void GameManager::DrawDeathScreen() {
    ...
    std::cout << "========================================================================\n";
    std::cout << "\n";
    LogEvent("GAME OVER: " + localUsername + " died in " + currentBiome + " at Level " + std::to_string(player->GetLevel()) + ".");
...
void GameManager::LogEvent(const std::string& message) {
    std::ofstream logFile("desolate_trek_log.txt", std::ios::app);
