...
void GameManager::HandleCombatTurn() {
    ...
    for (auto& combatant : turnOrder) {
        ...
        if (pChar) {
            ...
            for (int i = 0; i < entityParty.size(); ++i) {
                if (entityParty[i]->IsAlive()) {
                    aliveIndices.push_back(i);
                }
            }

            if (!aliveIndices.empty()) {
                int randomIndex = GetRandomTarget(aliveIndices.size());
                int targetIndex = aliveIndices[randomIndex];

                combatant->TakeTurn(enemies, entityParty); 
                
                std::cout << "\n > " << combatant->GetName() << " targets " 
                          << entityParty[targetIndex]->GetName() << "!\n";
            }
        }
