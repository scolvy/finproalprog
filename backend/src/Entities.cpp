...
std::string Character::PromptLocalTurn(std::vector<std::shared_ptr<Entity>>& allies, std::vector<std::shared_ptr<Entity>>& enemies) {
    ...
    while (!validTurn) {
        ...
        // Determine if we need ally or enemy target
        std::vector<std::shared_ptr<Entity>>* targetGroup = &enemies; // Default to enemies
        std::string targetPrompt = "Target Enemy";

        if (choice > 3) {
            // This is a skill; determine target type from skill
            SkillType skillType = skills[choice - 4].type;
            if (skillType == SKILL_HEALING || skillType == SKILL_BUFF) {
                targetGroup = &allies;
                targetPrompt = "Target Ally";
            }
        }

        std::cout << "> Select " << targetPrompt << " (Number): " << std::flush;
        std::cin >> targetIdx;

        if (targetIdx < 1 || targetIdx > targetGroup->size() || !(*targetGroup)[targetIdx - 1]->IsAlive()) {
            std::cout << "\n[!] Invalid target.\n";
            continue;
        }
...
void Character::ExecuteNetworkTurn(std::vector<std::shared_ptr<Entity>>& allies, std::vector<std::shared_ptr<Entity>>& enemies, int choice, int targetIdx) {
    ...
    std::vector<std::shared_ptr<Entity>>* targetGroup = &enemies;
    
    if (choice <= 3) {
        targetGroup = &enemies;
        target = (targetIdx >= 1 && targetIdx <= enemies.size()) ? enemies[targetIdx - 1] : nullptr;
    } else {
        // Determine target type from skill
        if (currentJob && targetIdx > 0) {
            const auto& skills = currentJob->GetSkills();
            if (choice - 4 < skills.size()) {
                SkillType skillType = skills[choice - 4].type;
                if (skillType == SKILL_HEALING || skillType == SKILL_BUFF) {
                    targetGroup = &allies;
                } else {
        ...
        target = (targetIdx >= 1 && targetIdx <= targetGroup->size()) ? (*targetGroup)[targetIdx - 1] : nullptr;
    }
    ...
    if (choice <= 3) {
        ...
        // Find the current character in the allies vector to pass the correct user
        std::shared_ptr<Entity> currentCharPtr = nullptr;
        ...
        if (currentCharPtr) {
            // Pass the entire enemies vector so AoE skills can hit all enemies
            currentJob->ExecuteSkill(choice - 4, currentCharPtr, allies, enemies);
        }
    }
}
