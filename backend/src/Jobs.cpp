...
Sparkmage::Sparkmage() {
    ...
    skillTree = {
        ...
        {"Meteor", 5, "Summons a devastating arcane meteor from the upper atmosphere. Deals 50 flat arcane damage to the entire enemy formation."}
    };
}
...
Juggernaut::Juggernaut() {
    ...
    skillTree = {
        ...
        {"Siege Breaker", 5, "A momentum-driven strike capable of shattering barricades. Deals 80 base damage to one target. Guaranteed to hit."}
    };
}
...
Voidcaller::Voidcaller() {
    ...
    skillTree = {
        ...
        {"Singularity", 5, "Collapses a local reality point. Deals 100 flat Arcane damage to all enemies, potentially wiping out entire groups."}
    };
}
...
FleshMender::FleshMender() {
    ...
    skillTree = {
        ...
        {"Resurrection", 5, "Channels forbidden energy into the body of a fallen ally. Restores the target with 1 HP if they are currently incapacitated."}
    };
}
void FleshMender::ExecuteSkill(int sIdx, std::shared_ptr<Entity> user, std::vector<std::shared_ptr<Entity>>& p, std::vector<std::shared_ptr<Entity>>& e) {
    ...
    else if (sIdx == 2) { for(auto& ally : p) if(!ally->IsAlive()) ally->Heal(1); user->SpendEnergy(5); }
}
...
BloodBrawler::BloodBrawler() {
    ...
    skillTree = {
        ...
        {"Blood God", 5, "Communes with the primal forces of vitality. Restores 50% of the user's maximum HP instantly."}
    };
}
